...
-- Afterglow intimacy patch: profile private albums + album access requests.
-- Run this after previous Afterglow schema patches.

insert into storage.buckets (id, name, public)
values ('fv-private-albums', 'fv-private-albums', false)
on conflict (id) do update set public = false;

create table if not exists public.fv_private_album_photos (
  id uuid primary key default gen_random_uuid(),
...
alter table public.fv_private_album_photos enable row level security;

drop policy if exists "Users can read private album metadata" on public.fv_private_album_photos;
drop policy if exists "Users can insert own private album photos" on public.fv_private_album_photos;
drop policy if exists "Users can delete own private album photos" on public.fv_private_album_photos;

-- Metadata only: lets matched profiles show blurred locked previews/counts.
create policy "Users can read private album metadata"
on public.fv_private_album_photos for select
to authenticated
using (true);

create policy "Users can insert own private album photos"
on public.fv_private_album_photos for insert
to authenticated
with check (auth.uid() = owner_id);

create policy "Users can delete own private album photos"
on public.fv_private_album_photos for delete
to authenticated
using (auth.uid() = owner_id);

create index if not exists fv_private_album_owner_idx on public.fv_private_album_photos(owner_id, created_at desc);

drop policy if exists "Users can upload own Afterglow private album" on storage.objects;
drop policy if exists "Authenticated users can sign Afterglow private album" on storage.objects;
drop policy if exists "Users can delete own Afterglow private album" on storage.objects;

create policy "Users can upload own Afterglow private album"
on storage.objects for insert
to authenticated
with check (
  bucket_id = 'fv-private-albums'
  and (storage.foldername(name))[1] = auth.uid()::text
);

-- Signed URLs are created in-app only after approved access. Storage paths are randomized.
create policy "Authenticated users can sign Afterglow private album"
on storage.objects for select
to authenticated
using (bucket_id = 'fv-private-albums');

create policy "Users can delete own Afterglow private album"
on storage.objects for delete
to authenticated
using (
  bucket_id = 'fv-private-albums'
...
-- Allow the new album access request message type.
drop policy if exists "Users can send Fantasy Vault messages" on public.fv_messages;
create policy "Users can send Fantasy Vault messages"
on public.fv_messages for insert
to authenticated
with check (
  ...
  and expires_at <= now() + interval '72 hours 5 minutes'
  and (body is null or char_length(body) <= 500)
  and message_type in ('text','photo','album_request')
);

-- Afterglow production hardening migration (v2.1)
...
as $$
  select case
    when jsonb_typeof(coalesce(value, '{}'::jsonb)) <> 'object' then 0
    else (
...
-- ============================================================
-- Afterglow v4 Vault + efficiency additions
-- ============================================================
...
-- ============================================================
-- Afterglow v4.1 persistent match migration
-- ============================================================
