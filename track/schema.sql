...
-- WeTrack V2.9.3: dedicated account-level onboarding / Traveler Passport defaults
-- IMPORTANT: itinerary_user_profiles is legacy/trip-scoped on some deployments, so
-- account onboarding state is deliberately stored in its own table.
...
-- WeTrack V2.9.3 safe starter packing seeder.
...
begin
  ...
  if not exists (select 1 from public.itinerary_trip_members m where m.trip_id=target_trip_id and m.user_id=auth.uid() and m.role in ('owner','editor')) then
    raise exception 'Not authorized';
  end if;
  perform pg_advisory_xact_lock(hashtext(target_trip_id::text || ':' || auth.uid()::text || ':packing'));
  if not exists (select 1 from public.itinerary_packing_items p where p.trip_id=target_trip_id and p.user_id=auth.uid()) then
  ...
  return query select p.* from public.itinerary_packing_items p where p.trip_id=target_trip_id and p.user_id=auth.uid() order by p.sort_order,p.created_at;
end;
$$;
...
-- FINAL AUTHORITATIVE LICENSING OVERRIDE — WeTrack V3.0
...
-- WeTrack V3.4 slideshow soundtrack fields
