...
-- v4: user-owned shared profile preferences for restaurant matching and surprise suggestions.
...
drop policy if exists "Members can read traveler profiles" on public.itinerary_user_profiles;
drop policy if exists "Members can insert own traveler profile" on public.itinerary_user_profiles;
drop policy if exists "Users can update own traveler profile" on public.itinerary_user_profiles;
drop policy if exists "Users can delete own traveler profile" on public.itinerary_user_profiles;

create policy "Members can read traveler profiles" on public.itinerary_user_profiles
for select using (public.user_can_view_trip(trip_id));

create policy "Members can insert own traveler profile" on public.itinerary_user_profiles
for insert with check (auth.uid() = user_id and public.user_can_view_trip(trip_id));

create policy "Users can update own traveler profile" on public.itinerary_user_profiles
for update using (auth.uid() = user_id and public.user_can_view_trip(trip_id)) with check (auth.uid() = user_id and public.user_can_view_trip(trip_id));

create policy "Users can delete own traveler profile" on public.itinerary_user_profiles
for delete using (auth.uid() = user_id);

create index if not exists itinerary_user_profiles_trip_id_idx on public.itinerary_user_profiles(trip_id);
create index if not exists itinerary_user_profiles_user_id_idx on public.itinerary_user_profiles(user_id);
