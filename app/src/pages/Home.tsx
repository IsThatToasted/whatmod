  ...
  return <div className="page">
    ...
    <section className="section-grid">
      <div className="panel wide"><div className="section-title"><div><p className="eyebrow">Jobs</p><h2>Projects</h2></div><Link to="/projects">View all <ArrowRight size={16}/></Link></div>
        <div className="project-list">{projects.slice(0,5).map(p => <Link className="project-row" to={`/projects?project=${p.id}`} key={p.id}><div><strong>{p.name}</strong><span>{p.address}{p.clientName?` · ${p.clientName}`:''}</span></div><div className="row-meta"><StatusPill status={p.status}/><span>{p.progress}%</span></div></Link>)}</div>
      </div>
