...
export function AuthGate({children}:{children:ReactNode}){
 ...
 const refreshMembership=async()=>{if(!supabase||!session?.user){setMembership(null);return} const {data,error}=await supabase.from('organization_members').select('organization_id,role,display_name,organizations(id,name,slug)').eq('user_id',session.user.id).eq('active',true).limit(1).maybeSingle(); if(error) debugError(AF_ERROR.authentication,error); const raw=(data as RawMembership|null)??null; const next:Membership|null=raw?{organization_id:raw.organization_id,role:raw.role,display_name:raw.display_name,organization:raw.organizations??null}:null; setMembership(next); if(next?.organization_id) await hydrateProjectsFromCloud(next.organization_id)};
...
function Login(){const [error,setError]=useState('');const signIn=async()=>{if(!supabase)return;const {error}=await supabase.auth.signInWithOAuth({provider:'google',options:{redirectTo:window.location.href}});if(error){debugError(AF_ERROR.authentication,error);setError(publicError(AF_ERROR.authentication,"We couldn't start sign in."))}};return <div className="auth-screen"><div className="auth-card"><p className="eyebrow">Construction operations</p><h1>Aurelium Field</h1><p>Projects, estimating, field documentation, crews and time in one workspace.</p><button className="button primary" onClick={signIn}>Continue with Google</button>{error&&<p className="error-text">{error}</p>}</div></div>}
function SetupNotice(){return <div className="auth-screen"><div className="auth-card"><h1>Service unavailable</h1><p>Aurelium Field could not connect to your workspace. Reference: <strong>AF-CFG-001</strong></p></div></div>}
function OrganizationOnboarding(){const {refreshMembership}=useAuth();const [mode,setMode]=useState<'create'|'join'>('create');const [name,setName]=useState('');const [invite,setInvite]=useState(new URLSearchParams(location.hash.split('?')[1]??'').get('invite')??'');const [error,setError]=useState('');const [busy,setBusy]=useState(false);
 const create=async()=>{if(!supabase)return;setBusy(true);const {error}=await supabase.rpc('create_organization',{org_name:name.trim()});setBusy(false);if(error){debugError(AF_ERROR.organizationCreate,error);setError(publicError(AF_ERROR.organizationCreate,"We couldn't create that organization."))}else await refreshMembership()};
 const join=async()=>{if(!supabase)return;setBusy(true);const token=invite.includes('invite=')?invite.split('invite=').pop()!:invite;const {error}=await supabase.rpc('accept_organization_invite',{invite_token:token.trim()});setBusy(false);if(error){debugError(AF_ERROR.organizationJoin,error);setError(publicError(AF_ERROR.organizationJoin,"We couldn't join that organization."))}else await refreshMembership()};
