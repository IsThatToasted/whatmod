...
type AuthValue={user:User|null;membership:Membership|null;isAdmin:boolean;refreshMembership:()=>Promise<void>;signOut:()=>Promise<void>};
...
export function AuthGate({children}:{children:ReactNode}){
 ...
 const signOut=async()=>{setMembership(null);setStartupFailed(false);try{await supabase?.auth.signOut({scope:'local'})}finally{setSession(null)}};
 const value=useMemo(()=>({user:session?.user??null,membership,isAdmin:['owner','admin'].includes(membership?.role??''),refreshMembership,signOut}),[session,membership]);
...
function OrganizationOnboarding(){const {refreshMembership,signOut}=useAuth();const [mode,setMode]=useState<'create'|'join'>('create');const [name,setName]=useState('');const [invite,setInvite]=useState(new URLSearchParams(location.hash.split('?')[1]??'').get('invite')??'');const [error,setError]=useState('');const [busy,setBusy]=useState(false);
