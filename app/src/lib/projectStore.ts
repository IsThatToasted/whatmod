...
export async function hydrateProjectsFromCloud(organizationID:string){
 if(!supabase)return;
 const {data,error}=await supabase.from('projects').select('id,name,client_name,address_line1,status,primary_trade,description,progress_percent,updated_at').eq('organization_id',organizationID).order('updated_at',{ascending:false});
 if(error){console.error(error);return}
 if(data){commit(data.map(row=>({id:row.id,name:row.name,address:row.address_line1??'',clientName:row.client_name??'',status:row.status,trade:row.primary_trade,progress:row.progress_percent??0,crewCount:0,updatedAt:new Date(row.updated_at).toLocaleDateString()})) as Project[])}
}
export function upsertProject(project:Project){
 ...
 void (async()=>{const org=await currentOrg();if(!org||!supabase)return;const {error}=await supabase.from('projects').upsert({id:project.id,organization_id:org.organizationID,name:project.name,client_name:project.clientName||null,address_line1:project.address||null,status:project.status,primary_trade:project.trade,description:null,progress_percent:project.progress,created_by:org.userID});if(error)console.error(error)})();
}
