...
async function reverseAddress(lat, lng) {
  // Nominatim often blocks direct browser requests from hosted domains via CORS.
  // Keep reverse lookup best-effort only; GPS coordinates are still enough for navigation links.
  const attempts = [
    async () => {
      ...
      if (!res.ok) throw new Error('Photon reverse lookup failed');
      const data = await res.json();
      const f = data.features?.[0];
      if (!f) return '';
      const pr = f.properties || {};
      return [pr.name, pr.street, pr.housenumber, pr.city, pr.state, pr.country].filter(Boolean).join(', ');
    },
    async () => {
      ...
      const res = await fetch(url);
      if (!res.ok) throw new Error('Nominatim reverse lookup failed');
      const data = await res.json();
...
async function geocodeWithPhoton(query) {
  ...
  const data = await res.json();
  const f = data.features?.[0];
  if (!f) throw new Error(`No result for ${query}`);
  const [lng, lat] = f.geometry.coordinates;
  const pr = f.properties || {};
  const label = [pr.name, pr.street, pr.housenumber, pr.city, pr.state, pr.country].filter(Boolean).join(', ') || query;
...
async function geocode(query) {
  ...
  if (!query) throw new Error('Enter a start and destination.');
  const coords = looksLikeCoordinates(query);
  if (coords) return { ...coords, label: query };

  // Hosted sites such as whatmod.com can be blocked by Nominatim CORS.
  // Photon is tried first because it is browser-friendly, then Nominatim is only a fallback.
  try { return await geocodeWithPhoton(query); }
  catch (photonErr) {
    try { return await geocodeWithNominatim(query); }
    catch (nomErr) {
      throw new Error(`Could not geocode “${query}”. Try a more specific address, city/state, place name, or coordinates.`);
    }
  }
}
