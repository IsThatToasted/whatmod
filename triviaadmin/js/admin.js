  ...
  try{
    ...
    $('#admin-app').innerHTML=`${adminHeader('questions')}<main class="shell">
      ...
      <section class="local-pipeline content-pipeline panel">
        <div class="pipeline-copy"><span class="eyebrow">LOCAL CONTENT STUDIO</span><h2>Questions + media in one upload.</h2><p>Acquire new Wikidata questions and resolve their media on your Windows PC. Export one content package, then upload that single file here. Canonical keys deduplicate reimports, admin edits are protected, and approved/locked media is preserved.</p></div>
...
async function openQuestionEditor(id){
  try{
    ...
    d.innerHTML=`<div class="drawer-backdrop"><aside class="drawer question-editor-drawer"><div class="drawer-head"><div><span class="eyebrow">${q.id?'EDIT QUESTION':'NEW MANUAL QUESTION'}</span><h2>${esc(q.prompt||'Create a question')}</h2>${q.id?`<div class="editor-votes"><span>👍 ${fmt(v.upvotes)}</span><span>👎 ${fmt(v.downvotes)}</span><span>${q.is_active?'ACTIVE':'RETIRED'}</span></div>`:''}</div><button class="x" id="close-question-editor">×</button></div>${questionFormMarkup(q)}${q.id?`<section class="panel record-meta"><h3>Record details</h3><dl><dt>ID</dt><dd>${esc(q.id)}</dd><dt>SOURCE</dt><dd>${esc(q.source_type||'manual')}</dd><dt>CANONICAL KEY</dt><dd>${esc(q.canonical_key||'—')}</dd><dt>CONTENT SYNC</dt><dd>${q.content_locked?'LOCKED BY ADMIN EDIT':'AUTOMATED UPDATES ALLOWED'}</dd><dt>CREATED</dt><dd>${esc(formatWhen(q.created_at))}</dd><dt>UPDATED</dt><dd>${esc(formatWhen(q.updated_at||q.created_at))}</dd>${q.deleted_at?`<dt>DELETED</dt><dd>${esc(formatWhen(q.deleted_at))}${q.delete_reason?` · ${esc(q.delete_reason)}`:''}</dd>`:''}</dl></section>`:''}</aside></div>`;
