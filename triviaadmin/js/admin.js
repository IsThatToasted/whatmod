...
async function renderLiveOps(){
  ...
  try{
    const data=await rpc('admin_live_operations_v15');
    const games=data?.games||[],users=data?.online_users||[];
    $('#admin-app').innerHTML=`${adminHeader('live')}<main class="shell"><section class="hero"><div><small>LIVE OPERATIONS</small><h1>Players online now.</h1><p>Presence is heartbeat-based. A player is considered online when the signed-in Trivia client has checked in within the last 90 seconds.</p></div><div class="live-generated"><span class="pulse">● LIVE</span><small>Snapshot ${esc(formatWhen(data?.generated_at))}</small></div></section>
