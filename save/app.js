...
async function ensureProfile(){
  ...
  const name=state.session.user.user_metadata?.full_name || state.session.user.user_metadata?.name || state.session.user.email?.split('@')[0] || 'Saver';
...
function renderLanding(){
  $('#app').innerHTML=`<main class="shell"><div class="topbar"><div class="brand"><div class="logo">S</div><div><h1>Save</h1><small>Shared money for shared plans</small></div></div></div><div class="wrap"><section class="hero"><div><div class="eyebrow">Trips • Tickets • Events • Goals</div><h2>Save together.<br>Spend fairly.</h2><p>Create a shared fund, automate everyone's contributions, track each person's fair share and vote to unlock more spending when the group needs it.</p><div class="ctaRow"><button class="btn primary" id="heroGoogle">Continue with Google</button><button class="btn" id="showEmail">Use email instead</button></div><div class="grid"><div class="card stat"><span class="muted">Shared goal</span><strong>$6,000</strong><span class="muted">Everyone sees the same progress.</span></div><div class="card stat"><span class="muted">Fair share</span><strong>$1,200</strong><span class="muted">Personal spend limits protect the group.</span></div><div class="card stat"><span class="muted">Unlock</span><strong>Vote</strong><span class="muted">Approve exceptions together.</span></div></div></div><div class="card authCard" id="authBox"><div class="eyebrow">Get started</div><h3 style="margin:0">Your group fund, one sign-in away</h3><p class="muted authIntro">Use your Google account for the fastest setup. Your name and profile image can be used inside shared funds.</p><button class="btn googleBtn" id="googleLogin"><span class="googleMark" aria-hidden="true">G</span><span>Continue with Google</span></button><div class="authDivider"><span>or continue with email</span></div><div id="emailAuth" class="emailAuth"><div class="field"><label>Email</label><input id="email" type="email" autocomplete="email" placeholder="you@example.com"></div><div class="field"><label>Password</label><input id="password" type="password" autocomplete="current-password" placeholder="6+ characters"></div><div class="authModeRow"><button class="textBtn active" id="modeLogin">Sign in</button><button class="textBtn" id="modeSignup">Create account</button></div><button class="btn primary" id="authSubmit">Sign in with email</button><button class="btn ghost" id="magicLink">Email me a magic link</button></div><div class="notice">Secure sign-in is handled by Supabase Auth. Save never receives your Google password.</div></div></section></div><div class="footer">Sandbox build • No real funds are moved</div></main>`;
  let mode='login';
  const setMode=(next)=>{mode=next;$('#modeLogin').classList.toggle('active',mode==='login');$('#modeSignup').classList.toggle('active',mode==='signup');$('#authSubmit').textContent=mode==='login'?'Sign in with email':'Create email account';};
  const googleSignIn=async()=>{
    const {error}=await supabase.auth.signInWithOAuth({provider:'google',options:{redirectTo:CONFIG.APP_URL}});
    if(error) toast(error.message,true);
  };
  $('#googleLogin').onclick=googleSignIn;
  $('#heroGoogle').onclick=googleSignIn;
  $('#showEmail').onclick=()=>{$('#email')?.focus();$('#authBox').scrollIntoView({behavior:'smooth',block:'center'});};
...
function topbar(){const avatar=state.session?.user?.user_metadata?.avatar_url || state.session?.user?.user_metadata?.picture;return `<div class="topbar"><div class="brand"><div class="logo">S</div><div><h1>Save</h1><small>Shared money for shared plans</small></div></div><div class="accountBar">${avatar?`<img class="profilePic" src="${esc(avatar)}" alt="">`:''}<span class="pill">${esc(state.profile?.display_name||'')}</span><button class="btn ghost" id="signOut">Sign out</button></div></div>`}
