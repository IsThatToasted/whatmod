...
-- Owner-only destructive action. Cascading foreign keys remove goal-scoped planning records atomically.
create or replace function public.delete_goal(p_fund_id uuid)
returns void language plpgsql security definer set search_path=public as $$
begin
  if auth.uid() is null then raise exception 'Not authenticated'; end if;
  if not exists(select 1 from public.funds where id=p_fund_id and owner_id=auth.uid()) then
    raise exception 'Only the goal owner can delete this goal';
  end if;
  delete from public.funds where id=p_fund_id and owner_id=auth.uid();
end $$;
...
drop policy if exists "members read commitments" on public.commitments;
...
drop policy if exists "members read commitment events" on public.commitment_events;
...
drop policy if exists "members read plans" on public.planned_expenses;
...
drop policy if exists "members read allocations" on public.plan_allocations;
...
drop policy if exists "members read calls" on public.funding_calls;
create policy "members read calls" on public.funding_calls for select to authenticated using (public.is_fund_member(fund_id));
drop policy if exists "members read call shares" on public.funding_call_shares;
