...
const categories: Array<[ItemType, RegExp]> = [
...
function inferDate(text: string) {
  ...
  if (/\b(this week|sometime this week|by friday)\b/.test(lower)) {
    const d = new Date(); const day = d.getDay(); const untilFriday = (5 - day + 7) % 7 || 7
    return addDaysISO(untilFriday)
  }
  const weekday = ['sunday','monday','tuesday','wednesday','thursday','friday','saturday']
  for (let i=0;i<weekday.length;i++) if (new RegExp(`\\b${weekday[i]}\\b`,'i').test(text)) {
    const d = new Date(); const delta = (i - d.getDay() + 7) % 7 || 7; return addDaysISO(delta)
  }
  return null
}
...
function inferTime(text: string) {
  ...
  if (match) {
    let h = Number(match[1]); const m = Number(match[2] || 0); const mer = match[3].toLowerCase()
  ...
  if (/\blunch\b/i.test(text)) return '12:00'
  return null
}
...
export class LifeIntentParser {
  parse(text: string): ParsedIntent {
    ...
    for (const [candidate, rx] of categories) if (rx.test(text)) { type = candidate; break }
    let priority: Priority = 'normal'
    if (/\b(urgent|important|asap|must)\b/i.test(text)) priority = 'high'
    if (/\b(no rush|whenever|low priority)\b/i.test(text)) priority = 'low'
