...
5. Verify the extension version is **1.3.0**.
...
## v1.3.0 quiet on-demand capture

The extension no longer parses shopping pages during normal browsing. It only inspects
the current page after you click **Add to JustGlance** (or click the toolbar action).
This keeps retailer pages passive and avoids unnecessary DOM work on search/category pages.

Best-effort parser/runtime failures are handled inside the panel instead of being logged
to the retailer page console. Errors generated by the retailer page, ad blockers, privacy
tools, or Chrome itself may still appear in DevTools because an extension should not
monkey-patch or suppress another site's console.
