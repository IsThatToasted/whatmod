import { FileText, FolderKanban, Link2, Search } from 'lucide-react'
...
export default function SearchPage() {
  const { items, notes, events, spaces, places, activity, captures } = useAppData()
  ...
  const results = useMemo(() => ({
    ...
    captures: term ? captures.filter(c => `${c.title} ${c.raw_text || ''} ${c.source_url || ''} ${c.file_name || ''} ${c.parsed_kind || ''}`.toLowerCase().includes(term)).slice(0, 12) : [],
  ...
  }), [term, items, projects, notes, captures, events, spaces, places, activity])
  ...
  return <div className="page search-page">
    ...
    {!term ? <div className="large-empty"><Search/><h2>Search your life.</h2><p>Tasks, projects, thoughts, files, links, appointments, spaces, places, shopping, and recent activity.</p></div> : count === 0 ? <div className="large-empty"><h2>No matches.</h2><p>Try a shorter phrase.</p></div> : <div className="search-results">
