...
export default function MemoryPage(){
  ...
  return <div className="page organizer-page memory-page">
    ...
    <div className="memory-toolbar"><div className="search-box compact-search memory-search"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search photos, files, links, people, text…"/></div><div className="chip-row memory-filters">{(['all','photos','files','links','text'] as Filter[]).map(v=><button key={v} className={`chip ${filter===v?'active':''}`} onClick={()=>setFilter(v)}>{v} <small>{counts[v]}</small></button>)}</div></div>
