...
import type { Project, ReminderRecord } from '../types'
...
interface OrganizerValue {
  ...
  createProject(input: { name: string; description?: string; target_date?: string | null; priority?: Project['priority']; space_id?: string | null }): Promise<Project>
  updateProject(id: string, patch: Partial<Project>): Promise<void>
  archiveProject(id: string): Promise<void>
  createReminder(input: { title: string; body?: string | null; remind_at: string; item_id?: string | null; event_id?: string | null; kind?: ReminderRecord['kind'] }): Promise<ReminderRecord>
  dismissReminder(id: string): Promise<void>
  deleteReminder(id: string): Promise<void>
}

const Ctx = createContext<OrganizerValue | null>(null)
const cacheKey = (uid: string) => `justglance:organizer:${uid}`
...
function notifyNative(reminder: ReminderRecord) {
  try {
    const handler = (window as any)?.webkit?.messageHandlers?.justglanceNative
...
function cancelNative(id: string) {
  try {
    const handler = (window as any)?.webkit?.messageHandlers?.justglanceNative
    if (handler?.postMessage) handler.postMessage({ type: 'cancelReminder', id })
  } catch {
...
export function OrganizerProvider({ children }: { children: ReactNode }) {
  ...
  const saveCache = useCallback((nextProjects: Project[], nextReminders: ReminderRecord[]) => {
    if (!userId) return
    try { localStorage.setItem(cacheKey(userId), JSON.stringify({ projects: nextProjects, reminders: nextReminders, saved_at: new Date().toISOString() })) } catch {}
  }, [userId])
  ...
  const refreshOrganizer = useCallback(async () => {
    ...
    try {
      ...
      const nextProjects = (p.data || []) as Project[]
      const nextReminders = (r.data || []) as ReminderRecord[]
      ...
      if (/projects|reminders|relation|schema cache/i.test(message)) setExpansionAvailable(false)
      try {
  ...
  useEffect(() => {
    if (demo || !supabase || !userId || !expansionAvailable) return
  ...
  const createProject: OrganizerValue['createProject'] = async input => {
    ...
    setProjects(next); saveCache(next, reminders)
    if (!demo && supabase) {
      const { error } = await supabase.from('projects').insert(row as never)
      if (error) { setExpansionAvailable(false); throw error }
    }
    return row
  }
  ...
  const updateProject = async (id: string, patch: Partial<Project>) => {
    ...
    setProjects(next); saveCache(next, reminders)
    if (!demo && supabase) {
      const { error } = await supabase.from('projects').update({ ...patch, updated_at: new Date().toISOString() } as never).eq('id', id)
      if (error) throw error
    }
  }

  const archiveProject = async (id: string) => updateProject(id, { status: 'archived', archived_at: new Date().toISOString() })

  const createReminder: OrganizerValue['createReminder'] = async input => {
    if (!userId) throw new Error('Not signed in')
    ...
    setReminders(next); saveCache(projects, next); notifyNative(row)
    if (!demo && supabase) {
  ...
  const dismissReminder = async (id: string) => {
    ...
    setReminders(next); saveCache(projects, next); cancelNative(id)
    if (!demo && supabase) {
      const { error } = await supabase.from('reminders').update({ dismissed_at: new Date().toISOString() } as never).eq('id', id)
      if (error) throw error
    }
  }
...
export function useOrganizer() {
  ...
  if (!value) throw new Error('useOrganizer must be used inside OrganizerProvider')
  return value
}
