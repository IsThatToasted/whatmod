import type { AppContextSnapshot, LifeItem, Mood } from '../types'
...
export function calculateRelevanceScore(item: LifeItem, context: AppContextSnapshot): ScoredItem {
  ...
  if (item.priority === 'high') { score += 15; reasons.push('high priority') }
  if ((item.estimated_minutes || 999) <= 10) { score += 10; reasons.push('quick win') }
  if (item.snooze_count >= 2) { score += 15; reasons.push('postponed') }

  const ageHours = (context.now.getTime() - new Date(item.created_at).getTime()) / 3600000
  if (ageHours <= 12) { score += 5; reasons.push('recent') }
  ...
  if (context.mood === 'quick' && (item.estimated_minutes || 999) <= 15) { score += 35; reasons.push('quick-mode fit') }
  if (context.mood === 'productive' && (item.priority === 'high' || item.energy_level === 'high')) { score += 25; reasons.push('productive-mode fit') }
...
export function rankItems(items: LifeItem[], context: AppContextSnapshot) {
  return items
...
export function matchesMoodFocus(item: LifeItem, mood: Mood | null) {
  if (!mood) return true
