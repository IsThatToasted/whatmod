...
> GitHub only executes workflows located at the **repository-root** `.github/workflows`. Workflow files are included under `life/.github/workflows` as safe templates so this project never silently replaces your existing whatmod.com Pages workflow. Copy the two build workflows to the root `.github/workflows`, or run `life/scripts_install_workflows.ps1` from Windows. Do **not** replace an existing Pages deployment workflow blindly.
...
`life/.github/workflows/justglance-web-build.yml` validates and packages `life/dist` without deploying the entire Pages site. This is the safe default for a multi-app repository.

If your existing whatmod.com deployment workflow already assembles the full site, add these steps to it:

```yaml
- uses: actions/setup-node@v4
  with:
    node-version: 22
- name: Build JustGlance
  working-directory: life
  run: npm install --no-audit --no-fund && npm run build
  env:
    VITE_SUPABASE_URL: ${{ secrets.JUSTGLANCE_SUPABASE_URL }}
    VITE_SUPABASE_ANON_KEY: ${{ secrets.JUSTGLANCE_SUPABASE_ANON_KEY }}
# Then copy life/dist into the final Pages artifact at _site/life/.
```

A static-repository example is included as `justglance-pages-static-template.yml`, but it is deliberately manual-only so it cannot unexpectedly replace your current Pages deployment.

### iOS unsigned IPA

Copy `justglance-ios-unsigned.yml` to the repository-root `.github/workflows`. It builds `life/ios/JustGlance.xcodeproj` on `macos-15`, disables signing, packages `Payload/JustGlance.app`, and uploads `JustGlance-unsigned.ipa` as a workflow artifact for sideload testing.
