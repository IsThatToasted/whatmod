export function contactPrimaryPhone(contact) {
    return contact?.phone_mobile || contact?.phone_work || contact?.phone_home || null;
}
export function contactPrimaryEmail(contact) {
    return contact?.email_personal || contact?.email_work || null;
}
export function contactPrimaryAddress(contact) {
...
function variants(contact) {
    ...
    if (contact.first_name?.trim())
        out.add(contact.first_name.trim());
    return [...out];
}
 ...
 */
export function resolveContactFromText(text, contacts) {
...
export function parseVCardContacts(text) {
    ...
    for (const block of blocks) {
        ...
        const findPhone = (needle) => phones.find(entry => needle.test(entry.params))?.value || null;
        const findEmail = (needle) => emails.find(entry => needle.test(entry.params))?.value || null;
        const findAddress = (needle) => addresses.find(entry => needle.test(entry.params))?.value || null;
        out.push({
