...
export function taskBucket(item) {
    ...
    if (item.is_inbox)
        return 'inbox';
    if (item.defer_until && item.defer_until > new Date().toISOString())
        return 'someday';
    if (item.due_date && item.due_date < today)
        return 'overdue';
    if (item.due_date === today || item.focus_pin)
        return 'today';
    if (item.due_date && item.due_date <= addDays(7))
        return 'upcoming';
    return 'anytime';
}
...
export function agendaForDate(date, items, events) {
