...
export function nextRecurrenceDate(rule, fromDate) {
    if (!rule)
        return null;
    const base = fromDate ? parseLocalDate(fromDate) : new Date();
    const normalized = rule.trim().toLowerCase();
    if (normalized === 'daily')
        return addDaysISO(1, base);
    if (normalized === 'weekly')
        return addDaysISO(7, base);
    if (normalized === 'biweekly')
        return addDaysISO(14, base);
    ...
    if (every)
        return addDaysISO(Math.max(1, Number(every[1])), base);
