...
function smartDate(text, fallback) {
    ...
    if (numeric) {
        ...
        if (!numeric[3] && candidate < new Date(new Date().setHours(0, 0, 0, 0)))
    ...
    if (written) {
        ...
        if (!written[3] && candidate < new Date(new Date().setHours(0, 0, 0, 0)))
            candidate.setFullYear(candidate.getFullYear() + 1);
        return isoLocalDate(candidate);
    }
    return fallback;
}
function smartTime(text, fallback) {
...
function cleanAppointmentTitle(text, location, url) {
    let title = text.trim();
    if (url)
    ...
    if (location)
        title = title.replace(new RegExp(`\\b(?:at|@)\\s+${location.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'i'), '');
...
export function analyzeSmartText(text) {
    ...
    if (isAppointment) {
        kind = 'appointment';
        reason = 'appointment language detected';
        confidence = Math.max(.86, confidence + .12);
    }
    else if (isLink) {
        kind = 'link';
        reason = 'web link detected';
        confidence = .92;
    }
    else if (isThought) {
        kind = 'thought';
        reason = 'thought/reference language detected';
        confidence = .82;
    }
    else if (intent.type === 'shopping') {
        kind = 'shopping';
        reason = 'shopping language detected';
    ...
    if (isLink && url) {
        ...
        try {
            title = withoutUrl || new URL(url).hostname.replace(/^www\./, '');
        }
        catch {
            title = withoutUrl || 'Saved link';
        }
    }
...
function parseIcsDate(value) {
    ...
    if (/^\d{8}$/.test(raw))
        return { date: `${raw.slice(0, 4)}-${raw.slice(4, 6)}-${raw.slice(6, 8)}`, time: '09:00' };
...
export function parseIcs(text, provider = 'ics') {
...
function calendarCsvDate(value) {
...
function calendarCsvTime(value, fallback = '09:00') {
    const clean = value.trim();
    if (!clean)
        return fallback;
...
export function parseCalendarCsv(text, provider = 'calendar-csv') {
    ...
    if (subjectIdx < 0 || startDateIdx < 0)
        return [];
    const events = [];
    ...
    if (isText && file.size <= 2_000_000) {
        ...
        if (isIcs) {
            ...
            if (result.events.length) {
                ...
                result.provider = likelyOutlook ? 'outlook-csv' : 'calendar-csv';
            }
        }
    }
    return result;
