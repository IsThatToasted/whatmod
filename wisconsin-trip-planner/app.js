...
function participantName(userId){ const m=state.members.find(x=>x.user_id===userId); return m?.profile?.display_name || (userId===state.user?.id ? displayName(state.profile,state.user) : 'Partner'); }
...
async function loadSessions(){
  const { data, error } = await sb.from('bcc_session_members').select('session_id, role, created_at, bcc_sessions(*)').eq('user_id', state.user.id).order('created_at',{ascending:false});
